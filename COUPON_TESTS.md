...
- Второй fixed купон -80₽ автоматически снят (недостаточно средств после корректировок)
- Скидка: 260₽ → 180₽
- Уведомление: "Купон 'Скидка -80₽ #2' автоматически снят, т.к. условия для его применения больше не выполняются"

**Неверно!!**
Верное поведение:
- Item купон остаётся
- Первый fixed купон -100₽ остаётся
- Второй fixed купон -80₽ остается, написано, что "Купон применен частично: 60 из 80"
- Скидка: 260₽ → 240₽

Потому что:
- после применения item купона:
240-80(купон на 2 подарка)-0(т.к. корректировка не применяется: эффективных подарка 4)=160
- после применения купона на -100:
160-100=60
- после применения купона на -80 должно быть:
"Купон применен частично: будет использовано 60 из 80"
Скидка = 80+100+60=260
...
Автоскидки: +45₽ (75% от (240-180-0) = 75% от 60)
ИТОГО: -15₽
```
...
Купоны: +240₽
  ...
  - Скидка -80p #2: 60
Корректировки: 0₽ (эффективно 6-2=4 подарка < 5)
Автоскидки: +0₽ (75% от (240-180-0-60) = 75% от 0)
ИТОГО: 0₽
```
