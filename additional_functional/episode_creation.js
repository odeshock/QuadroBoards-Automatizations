(function(){
  try{
    ...
    function castStr(){return selected.length ? '[FMVcast]'+selected.map(function(i){return i.code}).join(';')+'[/FMVcast]' : '';}
    function maskStr(){var pairs=[];selected.forEach(function(i){(i.masks||[]).forEach(function(m){pairs.push(i.code+'='+m)})});return pairs.length?'[FMVmask]'+pairs.join(';')+'[/FMVmask]':'';}
    ...
    function prefillFromTextarea(text){
      ...
      if(mm&&mm[1]) mm[1].split(';').forEach(function(p){var i=p.indexOf('='); if(i<=0)return; var c=p.slice(0,i).trim(), v=p.slice(i+1).trim(); if(!c||!v)return; (masks[c]||(masks[c]=[])).push(v);});
      var mp=text.match(/\[FMVplace\]([\s\S]*?)\[\/FMVplace\]/i); if(mp&&typeof mp[1]==='string') $placeInput.val(mp[1].trim());
      codes.forEach(function(code){ if(selected.some(function(x){return x.code===code}))return; var u=knownUsers.find(function(x){return x.code===code}); selected.push({code:code,name:(u?u.name:code),masks:(masks[code]||[])}); });
      Object.keys(masks).forEach(function(code){ if(selected.some(function(x){return x.code===code}))return; var u=knownUsers.find(function(x){return x.code===code}); selected.push({code:code,name:(u?u.name:code),masks:masks[code]}); });
    }
