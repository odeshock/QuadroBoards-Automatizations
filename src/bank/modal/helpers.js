...
/**
 * Создаёт портал для dropdown/suggest элемента с автопозиционированием
 * @param {HTMLElement} listElement - Элемент списка для портала
 * @param {HTMLElement} anchorElement - Элемент, к которому привязан портал (input)
 * @param {HTMLElement} containerElement - Контейнер для проверки кликов вне
 * @returns {Object} API для управления порталом
 */
export function createPortal(listElement, anchorElement, containerElement) {
  ...
  const positionPortal = () => {
    ...
    listElement.style.left = `${r.left}px`;
    listElement.style.top = `${r.bottom + 6}px`;
    listElement.style.width = `${r.width}px`;
  };
  ...
  const handleClickOutside = (e) => {
    if (!containerElement.contains(e.target) && !listElement.contains(e.target)) {
      close();
    }
  };
  ...
  const handleScroll = () => {
    if (listElement.style.display === 'block') positionPortal();
  };
