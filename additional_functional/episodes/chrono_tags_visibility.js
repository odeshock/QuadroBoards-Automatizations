// ==UserScript==
// @name         RusFF FMV: теги по кнопке (авто GroupID=1, поиск имён)
// @namespace    fmv-tools
// @match        https://testfmvoice.rusff.me/viewtopic.php?id=*
// @run-at       document-idle
// @grant        none
// ==/UserScript==

(function () {
  'use strict';

  /*** НАСТРОЙКИ ***/
  const REQUIRED_GROUP_ID = 1;      // показываем только этой группе
  const DEFAULT_COLLAPSED = true;   // блок по умолчанию свёрнут?
  const MAKE_NAMES_LINKS  = false;  // делать имена ссылками на профили

  // ─────────────────────────────────────────────────────────────────────────────
  //  Определение текущего GroupID (без хардкода): из HTML, потом из профиля
  // ─────────────────────────────────────────────────────────────────────────────

  async function getCurrentGroupId() {
    // 1) Попытка вытащить прямо со страницы: <script> / data-* / ссылки
    const fromPage = detectGroupIdInDocument(document);
    if (isFiniteNum(fromPage)) return fromPage;

    // 2) Попробуем взять ссылку «мой профиль» и вытащить group оттуда
    const myProfileLink = findOwnProfileLink();
    if (myProfileLink) {
      try {
        const html = await fetchHtml(myProfileLink.href);
        const doc  = new DOMParser().parseFromString(html, 'text/html');
        // сначала ищем прямые попадания в скриптах/атрибутах
        const inProf = detectGroupIdInDocument(doc);
        if (isFiniteNum(inProf)) return inProf;
        // затем ищем ссылку userlist.php?group=N
        const m = doc.documentElement.innerHTML.match(/userlist\.php\?group=(\d{1,3})/i);
        if (m) return Number(m[1]);
      } catch {}
    }
    return null;
  }

  function detectGroupIdInDocument(doc) {
    try {
      // a) data-* атрибуты
      const attrEl = doc.querySelector('[data-group-id],[data-gid],[data-user-group],[data-group]');
      if (attrEl) {
        const cand = attrEl.dataset.groupId || attrEl.dataset.gid || attrEl.dataset.userGroup || attrEl.dataset.group;
        if (isFiniteNum(cand)) return Number(cand);
      }

      // b) инлайновые скрипты: pun_user / g_id / group_id / gid
      const rePairs = [
        /\bpun_user\b[^{}]*\{[^}]*?\b(g_id|group_id|gid)\b\s*[:=]\s*(\d{1,3})/i,
        /\b(g_id|group_id|gid|user_group)\b\s*[:=]\s*["']?(\d{1,3})["']?/i
      ];
      const scripts = Array.from(doc.scripts).map(s => s.textContent || '');
      for (const txt of scripts) {
        for (const re of rePairs) {
          const m = txt.match(re);
          if (m) return Number(m[2]);
        }
      }

      // c) прямым регекспом по HTML (как крайний вариант)
      const html = doc.documentElement.innerHTML;
      let m = html.match(/\b(g_id|group_id|gid|user_group)\b\s*[:=]\s*["']?(\d{1,3})["']?/i);
      if (m) return Number(m[2]);
      m = html.match(/userlist\.php\?group=(\d{1,3})/i);
      if (m) return Number(m[1]);
    } catch {}
    return null;
  }

  function findOwnProfileLink() {
    // Ищем ссылку «Профиль» текущего пользователя в навигации/шапке
    const links = Array.from(document.querySelectorAll('a[href*="profile.php?id="]'));
    // приоритезируем по текстам и окрестностям
    const prefer = (a) => {
      const t = (a.textContent || '').trim();
      const title = (a.getAttribute('title') || '').trim();
      const near  = (a.closest('#pun-navlinks, #brdmenu, .top, .container, header, nav')?.textContent || '').trim();
      let score = 0;
      if (/Профил|Profile|Настройк|ЛК|Личный\s+раздел/i.test(t)) score += 5;
      if (/Профил|Profile/i.test(title)) score += 3;
      if (/Привет|Здравствуйте|Вы вошли|Личный/i.test(near)) score += 2;
      return score;
    };
    let best = null, bestScore = -1;
    for (const a of links) {
      try {
        const u = new URL(a.href, location.origin);
        if (!u.pathname.endsWith('/profile.php')) continue;
        const s = prefer(a);
        if (s > bestScore) { best = a; bestScore = s; }
      } catch {}
    }
    return best || null;
  }

  async function ensureAllowed() {
    const gid = await getCurrentGroupId();
    return gid === REQUIRED_GROUP_ID;
  }

  // ─────────────────────────────────────────────────────────────────────────────
  //  Импорт профиля и имён (cp1251/utf-8), подстановка userN → имя/красный
  // ─────────────────────────────────────────────────────────────────────────────

  // Профиль: сначала попробуем вытащить имя прямо со страницы темы (по ссылке на id)
  function getNameFromPageById(id) {
    const anchors = Array.from(document.querySelectorAll(`a[href*="profile.php?id=${id}"]`));
    for (const a of anchors) {
      const t = (a.textContent || '').trim();
      if (t && !/Профил|Profile/i.test(t)) return t;
    }
    return null;
  }

  async function fetchHtml(url) {
    const res = await fetch(url, { credentials: 'include' });
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const buf = await res.arrayBuffer();
    // RusFF часто cp1251
    let html = new TextDecoder('windows-1251').decode(buf);
    if (!/[А-Яа-яЁё]/.test(html)) html = new TextDecoder('utf-8').decode(buf);
    return html;
  }

  function extractNameFromDoc(doc) {
    const candidates = [
      '#pun-title span', 'h1 span', 'h1', 'h2.hn', '.hn', '.title', '.subhead h2', '.user-ident .username'
    ].map(sel => doc.querySelector(sel)).filter(Boolean)
     .map(el => (el.textContent || '').trim()).filter(Boolean);

    const title = (doc.querySelector('title')?.textContent || '').trim();
    if (title) candidates.unshift(title);

    for (let t of candidates) {
      t = t.replace(/\s+/g, ' ').trim();
      let m =
        t.match(/Профил[ья]\s*[:\-—–]\s*(.+)$/i) ||
        t.match(/Просмотр\s+профиля\s*[:\-—–]\s*(.+)$/i) ||
        t.match(/Profile\s*[:\-—–]\s*(.+)$/i);
      if (!m) m = t.match(/[—–-]\s*([^—–\-|]+)(?:[—–\-|].*)?$/);
      const raw = (m ? m[1] : t).trim();
      const clean = raw
        .replace(/[\s|–—-]+(?:Форум|Forum|FMV.*)$/i, '')
        .replace(/^[«"'\[]|[»"'\]]$/g, '')
        .trim();
      if (clean && !/Профил|Profile/i.test(clean)) return clean;
    }
    return null;
  }

  const nameCache = new Map();
  async function getProfileNameById(id) {
    if (nameCache.has(id)) return nameCache.get(id);
    let name = getNameFromPageById(id);
    if (!name) {
      try {
        const html = await fetchHtml(`/profile.php?id=${id}`);
        const doc  = new DOMParser().parseFromString(html, 'text/html');
        name = extractNameFromDoc(doc);
      } catch {}
    }
    nameCache.set(id, name || null);
    return name || null;
  }

  // ─────────────────────────────────────────────────────────────────────────────
  //  Разбор/замены userN
  // ─────────────────────────────────────────────────────────────────────────────
  function extractUserIdsFromString(s) {
    const ids = [];
    if (!s) return ids;
    const re = /user(\d+)/gi; let m;
    while ((m = re.exec(s))) ids.push(Number(m[1]));
    return ids;
  }
  function tokenToOutput(id, name) {
    if (name) {
      if (MAKE_NAMES_LINKS) return `<a href="/profile.php?id=${id}">${escapeHtml(name)}</a>`;
      return escapeHtml(name);
    }
    return `<span class="fmv-missing">user${id}</span>`;
  }
  function replaceUserTokens(text, idToNameMap) {
    if (!text) return '';
    return text.replace(/user(\d+)/gi, (_, num) => {
      const id = Number(num);
      return tokenToOutput(id, idToNameMap.get(id) || null);
    });
  }
  function replaceUserInPairs(text, idToNameMap) {
    if (!text) return '';
    return text.split(/\s*;\s*/).filter(Boolean).map(pair => {
      const [left, right] = pair.split('=');
      if (!right) return replaceUserTokens(left, idToNameMap);
      const replacedLeft = replaceUserTokens(left, idToNameMap);
      return `${replacedLeft}=${escapeHtml(right)}`;
    }).join('; ');
  }
  function escapeHtml(s) {
    return (s || '').replace(/[&<>"']/g, ch =>
      ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])
    );
  }
  const isFiniteNum = (x) => x !== null && x !== undefined && isFinite(Number(x));

  // ─────────────────────────────────────────────────────────────────────────────
  //  Вставка блока (маленькая кнопка ▸/▾ + запоминание состояния)
  // ─────────────────────────────────────────────────────────────────────────────
  let blockCounter = 0;
  function insertBlock(linesHtml, whereEl) {
    if (!linesHtml) return;

    const idx = ++blockCounter;
    const storageKey = `fmv:${location.pathname}${location.search}:#${idx}`;

    const container = document.createElement('div');
    container.className = 'fmv-meta is-collapsed';

    container.innerHTML = `
      <button class="fmv-toggle" type="button" aria-expanded="false" title="Показать/скрыть теги" data-key="${storageKey}">▸</button>
      <div class="fmv-body" hidden>
        ${linesHtml}
      </div>
    `;

    const styleId = 'fmv-meta-style';
    if (!document.getElementById(styleId)) {
      const st = document.createElement('style');
      st.id = styleId;
      st.textContent = `
        .fmv-meta{
          position: relative;
          border: 1px dashed #bfbfbf;
          padding: 10px 12px 10px 12px;
          margin: 10px 0 14px;
          font-size: 14px; line-height: 1.5;
          border-radius: 8px; background: #fafafa;
        }
        .fmv-meta.is-collapsed{ display:inline-block; border:none; padding:0; background:transparent; margin:6px 0; }
        .fmv-toggle{
          font: inherit; line-height: 1;
          border: 1px solid #d0d0d0; background: #f3f3f3;
          padding: 0 4px; border-radius: 6px; cursor: pointer; opacity: .7;
          user-select: none; height: 18px; min-width: 18px;
          display: inline-flex; align-items: center; justify-content: center;
        }
        .fmv-toggle:hover{ opacity: 1; }
        .fmv-body{ margin-top: 6px; }
        .fmv-row{ margin: 4px 0; }
        .fmv-label{ font-weight: 600; margin-right: .35em; }
        .fmv-missing{ background: #ffe5e5; color: #c00; padding: 0 .25em; border-radius: 4px; font-weight: 600; }
        .custom_tag.hidden_tag{ display: none !important; } /* исходные теги остаются скрытыми */
      `;
      document.head.appendChild(st);
    }

    if (whereEl) {
      whereEl.parentNode.insertBefore(container, whereEl.nextSibling);
    } else {
      const anchor = document.querySelector('.post, .blockpost, h1, #pun-title') || document.body.firstChild;
      anchor.parentNode.insertBefore(container, anchor);
    }

    const btn  = container.querySelector('.fmv-toggle');
    const body = container.querySelector('.fmv-body');
    const saved = localStorage.getItem(storageKey);
    const startCollapsed = saved === null ? DEFAULT_COLLAPSED : (saved === '1');
    applyCollapsed(startCollapsed);

    btn.addEventListener('click', () => {
      const collapsed = container.classList.toggle('is-collapsed');
      body.hidden = collapsed;
      btn.setAttribute('aria-expanded', String(!collapsed));
      btn.textContent = collapsed ? '▸' : '▾';
      localStorage.setItem(storageKey, collapsed ? '1' : '0');
    });

    function applyCollapsed(collapsed) {
      if (collapsed) {
        container.classList.add('is-collapsed');
        body.hidden = true;
        btn.setAttribute('aria-expanded', 'false');
        btn.textContent = '▸';
      } else {
        container.classList.remove('is-collapsed');
        body.hidden = false;
        btn.setAttribute('aria-expanded', 'true');
        btn.textContent = '▾';
      }
    }
  }

  // ─────────────────────────────────────────────────────────────────────────────
  //  Главная логика
  // ─────────────────────────────────────────────────────────────────────────────
  (async function main() {
    const allowed = await ensureAllowed();
    if (!allowed) return;

    const tagSelectors = [
      'characters.custom_tag_FMVcast, characters.custom_tag.custom_tag_FMVcast',
      'masks.custom_tag_FMVmask, masks.custom_tag.custom_tag_FMVmask',
      'location.custom_tag_FMVplace, location.custom_tag.custom_tag_FMVplace',
      'order.custom_tag_FMVord, order.custom_tag.custom_tag_FMVord'
    ].join(', ');
    const allTags = Array.from(document.querySelectorAll(tagSelectors));
    if (allTags.length === 0) return;

    // группируем по «постам»
    const byContainer = new Map();
    for (const tag of allTags) {
      const post = tag.closest('.post, .blockpost, .postmsg, .entry, .msg, .main, article') || document.body;
      if (!byContainer.has(post)) byContainer.set(post, []);
      byContainer.get(post).push(tag);
    }

    for (const [post, tags] of byContainer.entries()) {
      const charEl = tags.find(el => el.tagName.toLowerCase() === 'characters');
      const maskEl = tags.find(el => el.tagName.toLowerCase() === 'masks');
      const locEl  = tags.find(el => el.tagName.toLowerCase() === 'location');
      const ordEl  = tags.find(el => el.tagName.toLowerCase() === 'order');

      const charText = charEl?.textContent.trim() || '';
      const maskText = maskEl?.textContent.trim() || '';
      const locText  = locEl?.textContent.trim() || '';
      const ordText  = ordEl?.textContent.trim() || '';

      // собрать userN → заранее получить имена
      const ids = new Set([
        ...extractUserIdsFromString(charText),
        ...extractUserIdsFromString(maskText),
      ]);
      const idToNameMap = new Map();
      await Promise.all(
        Array.from(ids).map(async (id) => {
          const nm = await getProfileNameById(id);
          if (nm && nm.length) idToNameMap.set(id, nm);
        })
      );

      // строки вывода: каждая метка — новая строка
      const lines = [];
      if (charText) {
        const normalized = charText.split(/\s*;\s*/).filter(Boolean).join('; ');
        const nice = replaceUserTokens(normalized, idToNameMap);
        lines.push(`<div class="fmv-row"><span class="fmv-label">Состав:</span>${nice}</div>`);
      }
      if (maskText) {
        const pairs = replaceUserInPairs(maskText, idToNameMap);
        lines.push(`<div class="fmv-row"><span class="fmv-label">Маски:</span>${pairs}</div>`);
      }
      if (locText) {
        lines.push(`<div class="fmv-row"><span class="fmv-label">Локация:</span>${escapeHtml(locText)}</div>`);
      }
      if (ordText) {
        lines.push(`<div class="fmv-row"><span class="fmv-label">Порядок:</span>${escapeHtml(ordText)}</div>`);
      }

      if (lines.length) insertBlock(lines.join('\n'), tags[0]);
    }
  })();

})();
