...
### ⚠️ Тест 6.1: Автоскидка с одним купоном
**Начальная корзина:**
- 5 подарков × 40₽ = 200₽
...
Купоны: +100₽
  - Скидка -100₽: 100
Корректировки: -20₽ (5 подарков)
Автоскидки: +60₽ (75% от (200-100-20) = 75% от 80)
ИТОГО: -20₽
```

**Этот результат и наблюдаю при ручном тестировании**
...
1. Сначала применяются автом.скидки на конкретные расходы
Причем для каждой новой берется итоговая сумма после прежней автом.скидки
2. Потом применяются скидки на все расходы
Причем для каждой новой берется итоговая сумма после прежней автом.скидки
**Важное правило:** Если на каком-то этапе результат автом.скидок суммарно превышает текущие <расходы-купоны-корректировки>, то учитывается только часть (которая равноа расходы-купоны-корректировки)
Следующая автом.скидка уже не учитывается вообще

Т.е. тут должно было получиться:
1. 222 × 50% = 111
2. (120+222-111) x 75% = 231 x 75% = 174  (суммарно не превышает 120+222=342, поэтому эта скидка учитывается полностью)
3. Результат: 111+174=285 автом.скидка

Итого: -120-222+285 = -57
