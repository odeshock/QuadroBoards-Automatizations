// fmv-all-in-one.js — виджет + автоподключение + админ-переключатель
(function(){
  'use strict';
  ...
  function detectCurrentUser(){
    ...
    if (window.FMV && FMV.currentUser && (FMV.currentUser.id || FMV.currentUser.group)) {
      return { id: String(FMV.currentUser.id || ''), group: String(FMV.currentUser.group || '') };
    }
    // 2) <meta name="fmv-user-id" content="123"> / <meta name="fmv-user-group" content="1">
    const mu = document.querySelector('meta[name="fmv-user-id"]');
    const mg = document.querySelector('meta[name="fmv-user-group"]');
    const id = mu && mu.content ? String(mu.content) : '';
    const group = mg && mg.content ? String(mg.content) : '';
    if (id || group) return { id, group };
    // 3) пробуем достать id из ссылки на профиль
    const a = document.querySelector('a[href*="profile.php?id="]');
    if (a) {
      try {
        const u = new URL(a.href, location.origin);
        return { id: String(u.searchParams.get('id') || ''), group: '' };
      } catch(e){}
    }
    return { id: '', group: '' };
  }
  ...
  if (!window.FMV.fetchUsers) {
    console.warn('[FMV] Подключите модуль с FMV.fetchUsers до этого файла');
  }
  ...
  if (!window.FMV.UI) {
    (function(){
      // helpers
      function splitBySemicolon(s){ return String(s||'').split(';').map(v=>v.trim()).filter(Boolean); }
      function parseFMVcast(text){
        const mc = String(text||'').match(/\[FMVcast\]([\s\S]*?)\[\/FMVcast\]/i);
        const items = mc && mc[1] ? splitBySemicolon(mc[1]) : [];
        const by = {};
        items.forEach(tok=>{
          const m = tok.match(/^(user\d+)(?:=(.+))?$/i); if(!m) return;
          const code = m[1], mask = (m[2]||'').trim();
      ...
      function buildFMVplace(val){ val=String(val||'').trim(); return val ? '[FMVplace]'+val+'[/FMVplace]' : ''; }
      function buildFMVord(val){ let n=parseInt(String(val||'').trim(),10); if(Number.isNaN(n)) n=0; return '[FMVord]'+n+'[/FMVord]'; }
      ...
      function attach(opts){
        ...
        if (opts.stripOnMount) $area.val( stripFMV(initialRaw) );
        ...
        function renderAC(q){
          ...
          const items=knownUsers.filter(u=>!selected.some(x=>x.code===u.code))
  ...
  // ───────────────── Bootstraps + админ-переключатель ─────────────────
  ...
  onReady(function(){
    ...
    function attachToPage({strip=false, showOnlyIfCast=false}={}){
      ...
      if (!$form.length || !$area.length) return null;
      const api = FMV.UI.attach({ form:$form, textarea:$area, prefill:true, showOnlyIfFMVcast:!!showOnlyIfCast, className:'fmv--compact', stripOnMount:!!strip });
      return api ? { $form, $area, api } : null;
    }

    // где монтируем
    let ctx=null;
    // создание /post.php?fid=8|9 (старый маршрут)
    if (/\/post\.php$/i.test(path) && !q.has('action')) {
      const fid=+(q.get('fid')||0); if([8,9].includes(fid)) ctx=attachToPage({strip:false,showOnlyIfCast:false});
    }
    // редактирование первого /edit.php?topicpost=1
    if(!ctx && /\/edit\.php$/i.test(path) && q.get('topicpost')==='1'){ ctx=attachToPage({strip:true,showOnlyIfCast:true}); }
    // новый: создание /post.php?action=post&fid=8
    if(!ctx && /\/post\.php$/i.test(path) && q.get('action')==='post' && +q.get('fid')===8){ ctx=attachToPage({strip:true,showOnlyIfCast:false}); }
    // новый: редактирование /edit.php?id=N&action=edit (только если есть FMV-теги)
    if(!ctx && /\/edit\.php$/i.test(path) && q.get('action')==='edit' && q.has('id')){ ctx=attachToPage({strip:true,showOnlyIfCast:true}); }

    // ── Админ-кнопка (если админ и UI смонтирован) ──
    if (ctx && isAllowedAdmin()) {
      const {$form,$area} = ctx;
      const $placeBefore = ctx.api.mountPoint.length ? ctx.api.mountPoint : $area;
      const $tools = $('<div class="fmv-admin-tools"><button type="button" class="fmv-toggle">Режим ручного редактирования</button></div>');
      $placeBefore.before($tools);
      const $btn = $tools.find('.fmv-toggle');

      function toRaw(){
        const api = $form.data('fmvBoundUI'); // может отсутствовать
        const meta = api ? api.serialize() : '';
        const base = (function(txt){
          // локальный strip (на случай, если api отсутствует)
          return String(txt||'')
            .replace(/\[FMVcast\][\s\S]*?\[\/FMVcast\]/ig,'')
            .replace(/\[FMVplace\][\s\S]*?\[\/FMVplace\]/ig,'')
            .replace(/\[FMVord\][\s\S]*?\[\/FMVord\]/ig,'')
        ...
        if (api && api.mountPoint) api.mountPoint.remove();
